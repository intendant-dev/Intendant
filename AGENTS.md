...
Intendant is an autonomous AI agent operating environment written in Rust. It gives an AI agent a full desktop — shell, file editing, a graphical display it can see and control, voice, and phone calls — under layered human oversight. A daemon-wide Agenda preserves parked intent, reminders, durable questions, and digest-approved scheduled sessions; the Memory plane preserves explicitly retrieved, provenance-labeled claims over a vendored owner-plane writer/reducer kernel. Beyond running its own agent loop, Intendant **supervises external coding agents** (Codex, Claude Code, Kimi Code, Pi) as managed backends and **federates with peer machines**. It is provider-agnostic (OpenAI, Anthropic, Gemini) and cross-platform (macOS, Linux, Windows — all first-class). Its shipped trust anchors are local presence and an independently reached direct-mTLS dashboard; CLI and MCP provide automation, and the dashboard provides visual, voice, and phone control. The packaged macOS app contains a local mTLS bridge, but no Developer ID-signed/notarized release has been published for this alpha. An `-unsigned-dev` app artifact is not a distribution trust anchor; what is one is the publisher-PGP + transparency-log release lane — the compiled qualifying set's first entry, `macos-pgp-logged-v1`, evidence-verified per instance at enrollment — though nothing published yet can complete its verified install ceremony, so the signed-app lanes stay dormant behind honest refusals.

Past the single box, daemons federate into a **network of agentic networks** — fleets owned by people and organizations, where owners grant other people and other daemons scoped access to their machines, infrastructure, and resources. Three pillars carry it: the **trust architecture** (authority is minted only by the local authority of the governed resource — plane-root capabilities govern plane data, daemon-local IAM governs daemon effects, and Connect mints neither; a hosted claim is discovery/route metadata and creates no principal or grant; the default binary fixes hosted provenance at the zero-permission `role:none` and treats rendezvous-controlled fleet WebPKI names as discovery-only, so daemon control requires a trusted local surface, an independently reached daemon-served direct/mTLS surface, or — only where the owner has enabled the per-daemon hosted-control opt-in — an owner-approved hosted lease inside its compiled projection; org root keys sign grant documents and revocation lists), **credential custody** (sealed vault stores, time-boxed leases, and client egress exist, but the default build has no bridge from a Connect-origin account vault to a trusted daemon session; `.env` remains supported, and active full-credential OAuth leases temporarily materialize private auth files), and the **zero-install discovery client** (a browser tab can link a daemon with a single-use twelve-word claim code and find the fleet, but cannot control it). The trust stance is two layers — never compress them into one: the `role:none` hosted floor is the immutable all-off default, and the hosted convenience lane — daemon-minted, trusted-surface-approved expiring leases under a compiled floor and action wall (`docs/src/hosted-control.md`) — is **shipped** as the per-daemon `[connect] hosted_control_enabled` opt-in (restart-only, no fleet-wide switch; ratified 2026-07-16 as the default remote-control product). The wall keeps trust-critical verbs — approvals above all — off hosted surfaces at every preset. Connect remains self-hostable and is not an authority mint, but it is trusted for availability, account/route metadata, and the browser code it serves; the install scripts ship as release-pinned, transparency-logged GitHub release assets that Connect at most redirects to. The name is the thesis: agents perform, orchestrators conduct, the Intendant runs the house — and answers to the owner.
...
│   ├── auth_ceremony.rs, {codex,claude,kimi}_auth_ceremony.rs, global_store.rs
...
crates/intendant-custody     # custody backends: fail-closed trait, sealed blobs, keychain wrapping key (Track K)
