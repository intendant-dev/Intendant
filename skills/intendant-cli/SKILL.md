...
- Global `--peer ID` routes **any** ctl subcommand to a federated peer's `/mcp` over mTLS — no local daemon needed. It resolves the `[[peer]]` entry in `intendant.toml` by label, card_url host, or `intendant:<label>` id, then e.g. `ctl --peer dell display screenshot --output peer.png` or `ctl --peer dell cu actions --actions '[...]'` drive the peer's screen directly. The peer's IAM profile for this daemon decides: screenshots need display view (read-only-display or better), cu needs display input (peer-operator/peer-root). A denial is the peer's owner not having granted it — report, don't retry.
