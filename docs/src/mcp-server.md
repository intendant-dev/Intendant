...
stdio mode serves the thirteen `#[tool]`-router tools — `get_status`,
`get_logs`, `get_pending_approval`, `get_pending_input`, `approve`, `deny`,
`skip`, `approve_all`, `respond`, `set_autonomy`, `set_verbosity`, `quit`,
and `start_task` — plus `remote_command` on its SDK-backed owner surface.
It does not carry the daemon's Agenda or Memory service handles; the remaining
tools below are served by the wired daemon `/mcp` surface.
...
| `list_macos_windows` | Owner-only inventory of at most 16 retained AX objects, opaque candidate tokens and AX/CG identities for one explicit PID; requires an existing monitor helper and existing permissions. `DisplayView`; scoped user-display grants do not authorize listing. Facade: `inspect display windows PID`. | `pid` |
| `bind_macos_window` | Owner-only `DisplayInput`: bind an opaque listed candidate token plus matching PID/start-generation/window-ID identity to an exact owned monitor, transferring its retained AX object. Helper listing refresh (even failed) invalidates all unbound tokens across callers; selection consumes a token. No movement until place. Facade: `act display bind-window MONITOR CANDIDATE IDENTITY_JSON`. | `display_target`, `candidate`, `identity` |
| `place_macos_window` | Owner-only `DisplayInput`: monitor-local rectangle, strict containment, native identity/geometry/focus rechecks and AX/CG readback (one-point tolerance). Returns verified or partial with the latest observation; late failures explicitly mark effects unconfirmed and preserve available partial/verified results. No activation/input/retry/focus restoration. Facade: `act display place-window BINDING BOUNDS_JSON`. | `binding`, `bounds` |
| `unbind_macos_window` | Owner-only `DisplayInput`: release exact binding without closing or moving the window. Monitor destruction/EOF also release retained references. Facade: `act display unbind-window BINDING`. | `binding` |
| `read_macos_window_elements` | OwnerSurface + `DisplayView`: at most 16 retained semantic controls on the exact bound window wholly inside its unchanged owned monitor. Bounded roles/labels/bounds/operations/one-use tokens, no values or document text. Refresh/failure replaces the inventory; never starts a helper. | `binding` |
| `read_macos_window_keyboard_target` | OwnerSurface + `DisplayView`: read-only application-local focused AX receiver on the exact bound window, with strict before/after process/window/monitor/ancestry and human-focus witnesses. Returns only bounded role, bounds, enabled, and fixed `keyboard_dispatch_supported:false`; no label/value/text/secret/native pointer/token and no key-delivery claim. Never posts input, activates, changes focus or starts an unused helper. Facade: `inspect display keyboard-target BINDING`. | `binding` |
| `act_macos_window_element` | OwnerSurface + `DisplayInput`: one-use retained element action; tagged `type:press` or `type:set_value,text` (1024 UTF-8 bytes). Press is dispatched/effect-unverified; text requires exact private readback. No global input/focus restoration. [Contract and pending acceptance](../design/macos-bound-window-actions.md). | `binding`, `element`, `action` |
