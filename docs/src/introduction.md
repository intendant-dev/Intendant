...
It runs on **macOS, Linux, and Windows**, is **provider-agnostic** (OpenAI, Anthropic, Gemini), and is designed so that every capability is reachable from trusted interfaces — CLI, daemon-served web dashboard, MCP, voice, or the signed native app. Hosted Connect gives any browser zero-install route linking and discovery, but the default build fixes every hosted-provenance session at `role:none`; it is not a daemon-control interface.

Beyond the single machine, the goal is a **network of agentic networks**: daemons owned by people and organizations, where an owner grants other people — and other daemons — scoped IAM access to their machines, infrastructure, and resources ([Trust Architecture](./trust-architecture.md)). Credential-custody components include sealed vault storage, time-boxed leases, and browser client egress, but the default build does not yet bridge a Connect-origin account vault into a trusted direct/native daemon session. `.env` remains supported, and full-credential OAuth leases temporarily materialize private auth files ([Credential Custody](./credential-custody.md)).
...
- **Trust architecture** — daemon-local IAM (principals, grants, roles, ceilings), shipped local/native/direct-mTLS browser authentication, staged browser identity-key records for fleet signing and future identity work, passkey-protected account/vault envelopes, org root keys signing grant documents and revocation lists, and an append-only transparency log for hosted metadata ([Trust Architecture](./trust-architecture.md), [Self-Hosted Rendezvous](./self-hosted-rendezvous.md))
