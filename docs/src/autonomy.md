...
mutating or unknown tool enters this external-session approval rail; “approve
for session” is scoped to that Pi tool name and process. Missing UI, malformed
requests, extension errors, and future unknown tools are denied rather than
falling back to Pi's normal interactive policy.
