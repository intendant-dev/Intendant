...
Establish control from a trusted local, daemon-served direct/mTLS, or
signed-native surface. Hosted Connect remains discovery-only at immutable
...
- **`intendant access` enrollment is still Unix-validated.** Native HTTPS/WSS and
  ...
  `--tls-cert` / `--tls-key` plus `--mtls-ca`, or run enrollment from a Unix
  peer. A generic HTTPS reverse proxy is not an authentication substitute: if
  it terminates into daemon loopback it becomes a root trust boundary and must
  enforce approved client identity plus a protected upstream. See
