...
service worker is visible over CDP. The profile and extension tree share one
cleanup lifetime across launch failure, cancellation, display retirement, and
explicit close.
...
| `GET /api/external-agents` | External-agent backend availability (configured command, installed, auth posture, last used) plus passive zero-quota compatibility status (artifact fingerprint, in-band version, manifest digest, finding counts), each backend's install lane (matrix command + install state), and — for catalog-capable backends (Kimi) — the daemon-learned model catalog (`models` with capture provenance, or `models: null` + `models_reason` before any run) plus the compiled-baseline `compiled_suggestions` (vendored public-lineup aliases minus learned ids; suggestion-labeled fresh-install picker choices) — drives the fueling nudge, the new-session picker, every Kimi model picker, and the Vault Install buttons |
