...
**Path A: cursor as separate browser overlay (preferred for D-4+).** Peer sends `CursorState` frames on the control channel with `(x_px, y_px, visible)`. Browser maintains a small absolutely-positioned cursor sprite over the canvas. No tile dirtying needed for cursor motion. Matches RFB rich-cursor pseudo-encoding and RDP cursor shadow extension.

**Path B: cursor as synthetic dirty rects (D-3 fallback).** `SyntheticDirtySources` injects two dirty rects per cursor move (old position area + new position area), each one tile-size square centered on the cursor. The compositor draws cursor as part of the captured frame (the X server's normal compositing path). Simpler for D-3 because no separate overlay logic in the browser.

For D-3, use Path B (raw/RLE compositor draws everything). For D-4, switch to Path A as the encoding/policy work lands. Wire-level `CursorState` frame is defined now so the channel exists; D-3 just doesn't emit it.
...
| **D-3** | Real peer tile stream over real WebRTC datachannels: `tile/transport.rs` wire-format encode + chunking + backpressure + `webrtc.rs` data-channel registration + `display/mod.rs` integration of damage→synthetic→grid→encode→transport. **Raw BGRA + RLE encodings only** (no WebP, no VP8-intra). Periodic 30s snapshot. `TilePolicy` always returns `Tiles` (no fallback yet). Cursor handled via synthetic dirty rects (Path B). | View display from primary's dashboard against peer; canvas renders peer's actual desktop with cursor + window updates. ICE/DTLS/datachannel state stable for ≥5 min. Backpressure metrics (drop count, bufferedAmount) export and stay reasonable on idle desktop. |
| **D-4** | `tile/policy.rs` enabled (real fallback) + `FallbackToVideo`/`FallbackToTile` control messages + browser canvas/video toggle + cursor overlay path A (separate sprite) + WebP encoding (entropy-dispatched) + `GapReport` + ringbuffer recovery + Wayland/macOS frame-diff backend. Optional VP8-intra encoding gated behind a feature flag if browser support proves out. | Switching workloads on the peer (idle ↔ window drag ↔ video playback) triggers visible mode transitions; transitions stay below 1/sec under normal load. Snapshot recovery works after simulated channel drop. WebP encoded tiles render correctly when entropy threshold trips. |
