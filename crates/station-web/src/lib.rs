...
impl StationInner {
    ...
    fn draw_hud(&mut self, frame: &GpuFrame, time_ms: f64) {
        ...
        if self.width == 0 {
            self.keep_legacy_station_surfaces_typechecked(w, h, time_ms);
        }
    ...
    fn keep_legacy_station_surfaces_typechecked(&mut self, w: f32, h: f32, time_ms: f64) {
        self.draw_toolbar(w);
