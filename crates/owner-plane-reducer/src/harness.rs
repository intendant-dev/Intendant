...
/// The vendored corpus root (`corpus/` inside this crate): the stamped
/// spec, the companion schema, and `vectors/`.
...
mod tests {
    ...
    /// Vendoring integrity gate (added at vendor time, not part of the
    /// frozen source): the conformance below runs against the EXACT
    /// stamped spec — Gate A stamp `583f421a`, spec v0.5.24. Any byte
    /// drift in the vendored copy (including eol mangling; see
    /// .gitattributes) is a red test, never silent skew.
    #[test]
    fn vendored_spec_bytes_match_the_stamp() {
        use sha2::{Digest, Sha256};
        let bytes = std::fs::read(plane_root().join("owner-plane-d0a-spec.md")).unwrap();
        let hex: String = Sha256::digest(&bytes)
            .iter()
            .map(|b| format!("{b:02x}"))
            .collect();
        assert_eq!(
            hex, "2f20cceb85b5c6518eb27a35135b707c5a8aef5b711472b9e738b9daa6293d61",
            "vendored spec drifted from the Gate-A stamped bytes"
        );
    }
