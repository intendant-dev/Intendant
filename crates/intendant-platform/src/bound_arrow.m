// Fixed unmodified ArrowRight. Main-thread ARC/exception island.
...
#include <stdlib.h>
#include <unistd.h>
static const CGKeyCode ArrowKey = 124; // SDK Events.h: kVK_RightArrow
static NSString *arrow_text(void) {
...
static CGEventRef make_key(CGEventSourceRef source, CGEventType type,
    ...
    NSEvent *seed = [NSEvent keyEventWithType:type == kCGEventKeyDown ? NSEventTypeKeyDown : NSEventTypeKeyUp
        ...
        windowNumber:window context:nil characters:arrow_text()
        charactersIgnoringModifiers:arrow_text() isARepeat:NO keyCode:ArrowKey];
    ...
    CGEventSetIntegerValueField(event,kCGKeyboardEventKeycode,ArrowKey);
...
static BOOL matches(CGEventSourceRef source, CGEventRef event, CGEventType type,
    ...
    return decoded && decoded.windowNumber == window && decoded.keyCode == ArrowKey &&
        ...
        [decoded.characters isEqualToString:arrow_text()] &&
        CGEventGetType(event) == type && CGEventGetFlags(event) == 0 &&
        CGEventGetIntegerValueField(event,kCGKeyboardEventKeycode) == ArrowKey &&
...
void *intendant_arrow_create(int32_t pid,uint32_t window) {
    @autoreleasepool {
        if(![NSThread isMainThread] || pid<=0 || !window) return NULL;
        ...
        @try {
            ...
            p->down=make_key(p->source,kCGEventKeyDown,window,pid,tag);
            p->up=make_key(p->source,kCGEventKeyUp,window,pid,tag);
            if(matches(p->source,p->down,kCGEventKeyDown,window,pid,tag) &&
               matches(p->source,p->up,kCGEventKeyUp,window,pid,tag)) return p;
