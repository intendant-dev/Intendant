...
//!
//! Slice 1: the git segment. Fetch-free probes of a session's working
//! directory: branch, dirty count, ahead/behind vs `origin/<primary>`
//! (local fallback), a merge-parity preview via in-memory
//! `git merge-tree --write-tree` (git ≥ 2.38, cached by SHA pair), and
//! unpushed counts for the current and primary branches. A periodic
//! producer emits `AppEvent::SessionVitals` on change; frontends render
//! chips from the latest value and the session log replays it on
//! reconnect.
...
use crate::types::{SessionGitVitals, SessionVitals};
...
/// Periodic vitals producer for a fixed set of (session id, working dir)
/// targets — today the primary session. Emits on change only; the session
/// log persists each emission so reconnecting frontends replay the latest.
pub(crate) fn spawn_session_vitals_producer(
    ...
    tokio::spawn(async move {
        ...
        let mut last_emitted: HashMap<String, SessionVitals> = HashMap::new();
        loop {
            for (session_id, cwd) in &targets {
                let vitals = SessionVitals {
                    git: prober.probe(cwd).await,
                    cache: None,
                    limits: Vec::new(),
                };
                if vitals.git.is_none() && vitals.cache.is_none() && vitals.limits.is_empty() {
                    continue;
                }
                if last_emitted.get(session_id) == Some(&vitals) {
                    continue;
                }
                last_emitted.insert(session_id.clone(), vitals.clone());
                bus.send(AppEvent::SessionVitals {
