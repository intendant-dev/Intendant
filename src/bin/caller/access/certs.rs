...
/// SHA-256 fingerprint of the owner browser certificate generated by
/// `intendant access setup`. Unlike any other leaf signed by the access CA,
/// this specific local identity is eligible for the one-time direct-mTLS root
/// migration.
pub fn read_owner_client_cert_fingerprint(cert_dir: &Path) -> AccessResult<String> {
    let der = read_cert_der(&cert_dir.join(CLIENT_CRT))?;
    Ok(crate::access::access_policy::fingerprint_der(&der))
}
