...
pub const FRAME_LANES: &[FrameLaneSpec] = &[
    ...
    FrameLaneSpec {
        ...
        ws: true,
        tunnel: false,
        note: "SDP offer opening a ChatGPT-lane voice call; signaling relay into the voice broker only — no daemon credential is spent (subscription auth stays inside the Codex process)",
    },
    FrameLaneSpec {
        frame: "voice_stop",
        op: Some(PeerOperation::RuntimeControl),
        ws: true,
        tunnel: false,
        note: "stops the ChatGPT-lane voice call owned by this connection",
    },
    FrameLaneSpec {
        frame: "voice_usage",
        op: Some(PeerOperation::RuntimeControl),
        ws: true,
        tunnel: false,
        note: "provider-reported voice usage forwarded from the realtime data channel (decoration, never authoritative window state)",
    },
    FrameLaneSpec {
        frame: "voice_thread_purge",
        op: Some(PeerOperation::RuntimeControl),
        ws: true,
        tunnel: false,
        note: "owner purge lever for the durable voice presence thread (thread/delete + local identity reset)",
    },
    FrameLaneSpec {
...
pub fn control_msg_operation(ctrl: &ControlMsg) -> PeerOperation {
    match ctrl {
        ...
        // Reload-credentials is session lifecycle (interrupt + in-place
        // respawn), not credential access: the respawned backend reads
...
mod tests {
    ...
    fn realtime_frame_operation_golden_mapping_is_frozen() {
        ...
        const GOLDEN: &[(&str, Option<PeerOperation>, Option<PeerOperation>)] = &[
            ...
            ("voice_start",                    Some(RuntimeControl), None),
            ("voice_stop",                     Some(RuntimeControl), None),
            ("voice_usage",                    Some(RuntimeControl), None),
            ("voice_thread_purge",             Some(RuntimeControl), None),
