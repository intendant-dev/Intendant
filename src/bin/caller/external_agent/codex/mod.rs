...
- For GUI work in Intendant-managed sessions, use Intendant MCP tools directly, and prefer element-tree observation over screenshots: `execute_cu_actions` with `observe:"auto"` acts and returns the frontmost app's UI element tree in the same result (roles, labels, values, frames — a few hundred tokens; click the center of a reported frame; it falls back to a screenshot path when the tree is sparse and says which observation it returned). Use `read_screen` for a standalone element-tree look before acting, `take_screenshot` only when pixels are genuinely needed for visual verification, and `observe:"none"` when chaining action batches you will observe once at the end. Do not enumerate desktop apps or read bulky browser/computer-use plugin manuals when those direct tools are available; use Browser/Chrome/plugin CU only when their specialized capabilities are actually required. Do not use shell-driven GUI fallbacks such as `open`, `cliclick`, `osascript`, ad-hoc accessibility queries, or app binary inspection for GUI interaction.
