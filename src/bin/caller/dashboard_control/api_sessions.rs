...
pub(crate) async fn control_request_frame(
    ...
    match method.as_str() {
        ...
        "api_agenda_ref_content" => {
            api_agenda_ref_content_response(id, params.as_ref(), &runtime).await
        }
        ...
        "api_kimi_auth_start" => api_kimi_auth_start_response(id, params.as_ref(), &runtime).await,
        "api_kimi_auth_status" => api_kimi_auth_status_response(id, &runtime).await,
        "api_kimi_auth_cancel" => api_kimi_auth_cancel_response(id, &runtime).await,
...
/// Tunnel twin of `GET /api/agenda/items/{item_id}/refs/content` — the
/// item id rides `params.item_id`, the ref's locator `params.locator`;
/// reuses the transport-neutral core (sealed-when-pinned, live with the
/// drift verdict otherwise).
pub(crate) async fn api_agenda_ref_content_response(
    id: String,
    params: Option<&serde_json::Value>,
    runtime: &ControlRuntime,
) -> serde_json::Value {
    ...
    frame_api_response(
        id,
        crate::web_gateway::agenda_ref_content_api_response(
            &item_id,
            &locator,
            runtime.mcp_server.as_ref(),
        )
