...
async fn handle_session_current_changes(
...
async fn handle_current_history(
    mut stream: DemuxStream,
    file_watcher: Option<crate::file_watcher::SharedFileWatcher>,
) {
...
async fn handle_current_rollback(
    mut stream: DemuxStream,
    header_text: &str,
    bus: EventBus,
...
async fn handle_current_agent_output(
    mut stream: DemuxStream,
    header_text: &str,
    query_ctx: Option<WebQueryCtx>,
...
fn access_iam_update_grant_response_value_with_cert_dir(
    cert_dir: &std::path::Path,
    params: serde_json::Value,
    actor: &crate::access::iam::AccessPrincipal,
) -> Result<serde_json::Value, String> {
...
fn access_iam_update_grant_response_body(
    body_text: &str,
    actor: &crate::access::iam::AccessPrincipal,
) -> (u16, String) {
    let params = match serde_json::from_str::<serde_json::Value>(body_text) {
