...
function stationOperateBrowserWorkspace(op) {
  ...
  if (op === 'create') {
    ...
    dispatchDashboardActionMsg({
      ...
      // The old `currentSessionId` global died in the AppWeb→PresenceWeb
      // merge (9285c7b6); the bare read threw ReferenceError and silently
      // broke Station's workspace create. Same fallback chain as the
      // Debug pane's debugPaneSessionId() (PR #70 fix).
      owner_session_id: currentSessionFullId || daemonSessionFullId || undefined,
    });
