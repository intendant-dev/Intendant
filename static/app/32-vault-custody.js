...
const VAULT_OAUTH_PROVIDERS = {
  'oauth:openai-chatgpt': {
    ...
    // OpenAI's public Codex OAuth client, used here by Intendant Native.
...
function vaultOauthRefreshTokenOf(kind, secretJson) {
  if (kind === 'oauth:openai-chatgpt') {
    return String(secretJson?.refresh_token || secretJson?.tokens?.refresh_token || '');
  }
...
function vaultOauthExpiryMs(kind, secretJson) {
  ...
  if (kind === 'oauth:openai-chatgpt' && Number(secretJson?.expires_at_unix_ms) > 0) {
    return Number(secretJson.expires_at_unix_ms);
  }
  if (kind === 'oauth:codex' || kind === 'oauth:openai-chatgpt') {
    try {
      // ChatGPT-plan access tokens are JWTs; exp is authoritative and
      // survives page reloads (unlike a refresh-time bookkeeping map).
      const accessToken = kind === 'oauth:codex'
        ? secretJson?.tokens?.access_token
        : secretJson?.access_token || secretJson?.tokens?.access_token;
...
async function vaultOauthRefresh(kind, entry) {
  ...
  if (kind === 'oauth:openai-chatgpt') {
    ...
    if (fresh.expires_in) {
      secretJson.expires_at_unix_ms = nowMs + Number(fresh.expires_in) * 1000;
    } else {
      // Let the JWT exp claim become authoritative instead of retaining a
      // stale explicit expiry from the previous token.
      delete secretJson.expires_at_unix_ms;
    }
    secretJson.updated_at_unix_ms = nowMs;
  } else if (kind === 'oauth:codex') {
...
function vaultEntryLeaseKind(entry) {
  ...
  if (
    entry.kind === 'oauth' &&
    ['openai-chatgpt', 'codex', 'claude-code', 'kimi', 'pi'].includes(entry.provider)
  ) {
...
function agentSigninProviderCard(provider) {
  ...
  if (phase === 'success') {
    ...
    // The value half: LIVE sessions — running, parked between turns, or
...
function vaultProviderLabel(provider) {
  ...
  if (provider === 'openai-chatgpt') return 'Intendant Native (ChatGPT subscription)';
...
function vaultRenderAddForm(card) {
  ...
  const fillProviders = () => {
    ...
    const providers = kindSelect.value === 'oauth'
      ? ['openai-chatgpt', 'codex', 'claude-code', 'kimi', 'pi']
  ...
  secretArea.rows = 4;
  secretArea.placeholder = 'Paste the auth JSON (Intendant Native: <state-root>/auth/openai-chatgpt.json · Codex: ~/.codex/auth.json · Claude Code: ~/.claude/.credentials.json · Kimi: ~/.kimi-code/credentials/kimi-code.json · Pi: ~/.pi/agent/auth.json)';
...
async function githubIntegrationRefresh(force = false) {
  ...
  try {
    ...
    if (githubCeremonyReturn && body && body.status === 'valid') {
      // The chip reads valid on ceremony return: the explicit success
      // moment, with what happens next — never a silent state flip.
      githubCeremonyReturn = false;
      githubIntegrationState.notice = {
        text: `Connected — PR cards appear on the agenda within ~${body.poll_minutes || 5} minutes.`,
        cls: 'is-success',
      };
    }
  } catch (e) {
...
// Return-to-context (UX0 ruling): the callback page sends the owner
// back to `/?ceremony=github#vault`. On arrival: bypass the 30 s status
// cache (finding 3's stale paint), scroll this section into view once
// the deferred vault pane mounts it, and strip the marker from the URL
// so a reload doesn't replay the gesture.
let githubCeremonyReturn = false;
function githubIntegrationCeremonyReturnBoot() {
  let params;
  try { params = new URLSearchParams(window.location.search); } catch { return; }
  if (params.get('ceremony') !== 'github') return;
  githubCeremonyReturn = true;
...
}
{
  ...
  if (document.readyState !== 'loading') bootCeremonyReturn();
  else document.addEventListener('DOMContentLoaded', bootCeremonyReturn, { once: true });
}
...
async function githubIntegrationCompleteInstall(row) {
  ...
  try {
    ...
    githubIntegrationState.notice = { text: String(e?.message || e), cls: 'is-refusal' };
  }
...
async function githubIntegrationSaveSelection(payload) {
  ...
  try {
    ...
    githubIntegrationState.notice = {
      text: status === 'valid'
        ? `Watch list saved — verified against GitHub. PR cards appear on the agenda within ~${body?.poll_minutes || 5} minutes.`
        : `Saved. Status: ${status}${body?.detail ? ` — ${body.detail}` : ''}`,
      cls: status === 'valid' ? 'is-success' : 'is-refusal',
    };
  } catch (e) {
    githubIntegrationState.notice = { text: String(e?.message || e), cls: 'is-refusal' };
  }
...
async function githubIntegrationRemove() {
  ...
  try {
    ...
    githubIntegrationState.data = body || githubIntegrationState.data;
    githubIntegrationState.fetchedAt = Date.now();
    githubIntegrationState.notice = { text: 'Credentials removed from custody.', cls: 'is-success' };
  } catch (e) {
    githubIntegrationState.notice = { text: String(e?.message || e), cls: 'is-refusal' };
  }
  githubIntegrationState.busy = false;
...
function renderGithubIntegrationSection() {
  ...
  if (data && !data.configured) {
    ...
    if (data.app_slug) {
      ...
      // The single next action, presented as the primary button (the
      // NEEDS-YOU grammar): the ceremony waits on exactly this click.
      const link = document.createElement('a');
      link.className = 'vault-install-link primary';
    ...
    if (d.polling && found.length === 0) {
      // Ceremony in flight, nothing for the user to do: PROGRESS, with
      // its pulse — never the warning class.
      hint.className = 'vault-connect-hint is-progress';
      hint.textContent = 'The App is created and its key is sealed. Waiting for the installation to appear on GitHub…';
    } else if (found.length > 1) {
      hint.className = 'vault-connect-hint is-attention';
      hint.textContent = 'Several installations found — pick the one to watch:';
    } else if (d.error) {
      hint.className = 'vault-connect-hint is-refusal';
