...
function agendaGraphDraw(ts) {
  ...
  // Place-link bundling: a hub with a wide fan (≥6 children) routes
  // its links through a shared trunk — each curve's control point
  // pulls toward a point between the hub and its children's centroid,
  // so the fan leaves the hub as one limb and splays near the leaves
  // instead of spraying straight spokes. Screen-space, recomputed per
  // frame from live positions; smaller fans stay straight lines.
  const bundleTrunks = new Map();
  agendaGraphLinks.forEach((link) => {
    if (link.t !== 'place') return;
    let trunk = bundleTrunks.get(link.b);
    if (!trunk) {
      trunk = { x: 0, y: 0, n: 0 };
      bundleTrunks.set(link.b, trunk);
    }
    trunk.x += pts[link.a].x;
    trunk.y += pts[link.a].y;
    trunk.n += 1;
  });
  ...
  agendaGraphLinks.forEach((link) => {
    ...
    g.beginPath();
    g.moveTo(a.x, a.y);
    const trunk = link.t === 'place' ? bundleTrunks.get(link.b) : null;
    if (trunk && trunk.n >= 6) {
      const tx = b.x + (trunk.x / trunk.n - b.x) * 0.35;
      const ty = b.y + (trunk.y / trunk.n - b.y) * 0.35;
      const mx = (a.x + b.x) / 2;
      const my = (a.y + b.y) / 2;
      g.quadraticCurveTo(mx + (tx - mx) * 0.55, my + (ty - my) * 0.55, b.x, b.y);
    } else {
      g.lineTo(b.x, b.y);
    }
    if (link.t === 'place') {
