...
function accessFallbackIamRoles() {
  ...
  return [{
    ...
    permissions: ['presence.read', 'stats.read', 'display.view', 'access.inspect', 'peer.inspect', 'session.inspect', 'agenda.read', 'memory.read'],
    source: 'builtin',
  }, {
...
function accessFallbackPermissions() {
  const summaries = {
    ...
    'feedback.write': ['Developer dogfood feedback', 'feedback', 'Submit bounded developer feedback only when the daemon explicitly opts in; disabled by default, with no generic Agenda mutation authority.'],
