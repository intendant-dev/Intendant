  ...
  closableLens: {
    ...
    vectors: () => {
      ...
      const rows = [
        ...
        // Interrupted-arc amendment rows: unconcluded-arc evidence beats
        // the settled all-clear; a concluded arc falls through to it.
        ['close/interrupted-settled-unattested',
          t(settled({ phase: 'interrupted', hardDone: true, interrupted: true, attested: '' })),
          'Interrupted mid-work — resumable. Closing hides the last handle on it (agenda debt settled, but no achieved attestation)'],
        ['close/interrupted-stranded-tree',
          t(settled({ phase: 'interrupted', hardDone: true, interrupted: true, attested: 'achieved', worktreeStranded: true })),
          'Interrupted mid-work — resumable. Closing hides the last handle on it (agenda debt settled, but worktree holds uncommitted or unpushed work)'],
        ['close/interrupted-linkless',
          t({ phase: 'interrupted', hardDone: true, interrupted: true, attested: '' }),
          'Interrupted mid-work — resumable. Closing hides the last handle on it (no achieved attestation)'],
        ['close/interrupted-concluded-settled',
          t(settled({ phase: 'interrupted', hardDone: true, interrupted: true, attested: 'achieved', worktreeStranded: false })),
          'No agenda-owed work — the linked occurrence is settled'],
...
function ui2TagEntry(e) {
  ...
  if (/^\[(codex|claude(-code)?|kimi(-code)?|pi|backend) stderr\]/i.test(text)) {
