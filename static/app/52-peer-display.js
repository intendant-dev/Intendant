...
// Authority continuity: how long a pane keeps auto-resuming a held
// federated input-authority grant across reconnects. Mirrors the
// peer-side FEDERATED_AUTHORITY_CONTINUITY_WINDOW
// (web_gateway/input_authority.rs) — the peer is the enforcement point
// (an automatic resume request is refused outside its window, and never
// displaces a live holder); this browser-side deadline only stops the
// pane from re-asking after the peer would refuse anyway.
const PEER_AUTHORITY_RESUME_WINDOW_MS = 45000;
...
class PeerDisplayConnection {
  constructor(hostId, displayId, sessionId, advertiseTcpViaUrl) {
    ...
    // Authority continuity across transport churn (the federated encoder
    // ladder's slot rebuilds reconnect the whole pane): set on the
    // SUCCESSOR connection by openPeerDisplay when the pane it replaces
    // held federated input authority ('you') — a standing user grant the
    // user never released — or was itself still mid-resume. While set,
    // the first authoritative non-'you' state on the new authority
    // channel triggers an automatic `resume: true` re-request; the peer
    // only honors it inside its continuity window and never against a
    // live holder, so this can't steal control that moved elsewhere
    // during the reconnect. Cleared on grant ('you'), on explicit
    // releaseControl(), and when the deadline lapses.
    this._pendingAuthorityResume = false;
    this._authorityResumeDeadline = 0;
  ...
  async connect() {
    ...
    //   { "t": "display_input_authority_request", "display_id": N, "resume": true }
    //   { "t": "display_input_authority_release", "display_id": N }
    //   { "t": "display_input_authority_state",   "display_id": N, "state": "you|other|unclaimed" }
    // (`resume: true` marks the automatic continuity re-request sent by
    // `_attemptAuthorityResume`; peers predating it read a plain request.)
    //
  ...
  setPeerAuthorityState(state) {
    ...
    // Authority continuity: a grant consumes the carried resume intent;
    // any other authoritative state while it's armed is the cue to
    // re-assert the user's standing grant on this successor connection.
    // Attempts are strictly state-push-driven (at most one in flight),
    // so a peer-side refusal — which sends nothing — can't loop.
    if (state === 'you') {
      this._pendingAuthorityResume = false;
      this._authorityResumeDeadline = 0;
    } else if (this._pendingAuthorityResume) {
      this._attemptAuthorityResume(state);
    }
  ...
  takeControl() {
    ...
    this._armTakePendingTimer(false);
  }

  // Item 7b timer, shared by the user click (takeControl → toast on
  // silence) and the automatic continuity re-request (silent: an unmet
  // automatic attempt is not the user's failure to confess by toast —
  // the chip already shows the truthful state).
  _armTakePendingTimer(silent) {
    ...
    this._takePendingTimer = window.setTimeout(() => {
      ...
      this._takeControlPending = false;
      if (!silent && typeof showControlToast === 'function') {
        showControlToast('error', 'The peer did not answer the input-control request — try again');
      }
    }, DISPLAY_VIEWER_TAKE_PENDING_TIMEOUT_MS);
  }

  // Authority continuity: re-assert a carried, still-standing user grant
  // from this successor connection. Sent with `resume: true`, which the
  // peer honors only inside the continuity window minted when the
  // predecessor's held grant was torn down by transport churn — and never
  // against a live holder — so this automatic frame can't win an
  // arbitration the user's click didn't already win. Called from
  // setPeerAuthorityState on each authoritative non-'you' push while the
  // intent is armed: the first push is usually 'unclaimed' (peer released
  // on the predecessor's close) and resumes immediately; an 'other' push
  // can be the predecessor's not-yet-reaped ghost after an unclean drop —
  // the peer refuses that attempt silently and the follow-up 'unclaimed'
  // broadcast (when the ghost reaps) triggers the attempt that lands.
  _attemptAuthorityResume(state) {
    if (Date.now() > this._authorityResumeDeadline) {
      this._pendingAuthorityResume = false;
      this._authorityResumeDeadline = 0;
      this._log('info', 'input-authority resume window expired — Take Control needs a fresh click');
      return;
    }
    if (this._takeControlPending) return; // an attempt is already in flight
    this._takeControlPending = true;
    if (!this._sendAuthorityFrame('display_input_authority_request', { resume: true })) {
      this._takeControlPending = false;
      return;
    }
    this._log('info', `auto-resuming held input authority (state was '${state}')`);
    this._armTakePendingTimer(true);
  }
  ...
  releaseControl() {
    ...
    this._takeControlPending = false;
    // Explicit user release ends the standing grant — never auto-resume
    // past it. (The peer-side release also voids its continuity window.)
    this._pendingAuthorityResume = false;
    this._authorityResumeDeadline = 0;
  }
  ...
  // `request_display_input_authority` WS path. `extra` merges
  // additional fields into the frame (the continuity path's
  // `{ resume: true }` marker).
  _sendAuthorityFrame(t, extra) {
...
async function openPeerDisplay(hostId, displayId, advertiseTcpViaUrl) {
  // Authority continuity: every reconnect (auto-retry, manual Retry, a
  // re-click of "View display") replaces the PeerDisplayConnection —
  // fresh session id — via this path. If the pane being replaced held
  // federated input authority ('you' — a standing user grant that was
  // never released) or was itself still mid-resume, carry that intent to
  // the successor so it can auto-re-request control once its authority
  // channel reports the post-churn state. The deadline carries across
  // chained reconnects — the clock runs from the first interruption of an
  // actually-held grant, matching the peer-side continuity window.
  let resumeAuthority = false;
  let resumeDeadline = 0;
  for (const conn of peerDisplayConnections.values()) {
    if (conn.hostId !== hostId || Number(conn.displayId) !== Number(displayId)) continue;
    if (conn.peerAuthorityState === 'you') {
      resumeAuthority = true;
      resumeDeadline = Date.now() + PEER_AUTHORITY_RESUME_WINDOW_MS;
    } else if (conn._pendingAuthorityResume) {
      resumeAuthority = true;
      resumeDeadline = conn._authorityResumeDeadline;
    }
  }
  ...
  if (resumeAuthority && resumeDeadline > Date.now()) {
    conn._pendingAuthorityResume = true;
    conn._authorityResumeDeadline = resumeDeadline;
    conn._log('info', 'carrying held input authority across the reconnect — will auto-resume');
  }
  ...
  peerDisplays() {
    return Array.from(peerDisplayConnections.values()).map(conn => {
      ...
      return {
        ...
        pendingAuthorityResume: Boolean(conn._pendingAuthorityResume),
      };
    });
  },
