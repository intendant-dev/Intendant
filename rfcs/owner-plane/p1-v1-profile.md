...
| Frontier-close head validation (carried heads vs held ops) | ~~markers ×6~~ **DONE** — every frontierclose site routes through the D-93 validator (unheld-Head pends, mismatched-hash rejects) with D-143 exactness on compound cutoffs; vectored (`f07-revoke-cutoff-carried-head-completes` / `-head-hash-mismatch-rejects` / `-empty-heads-with-history-rejects`). Boundary-retirement effects stay Gate-B sagas (D-203) | Cutoff ceremonies are the revocation backbone; head validation is their integrity. |
